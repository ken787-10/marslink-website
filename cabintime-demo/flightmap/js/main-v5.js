...
function calculateBearing(start, end) {
    ...
    const y = Math.sin(dLng) * Math.cos(end.lat);
    const x = Math.cos(start.lat) * Math.sin(end.lat) - Math.sin(start.lat) * Math.cos(end.lat) * Math.cos(dLng);
